...
const workflow_route_tuples = [
  ...
  [
    Router.get,
    "/workflow/create/summary/:blueprint_name",
    summarizeBlueprint_controller,
  ],
  [Router.get, "/workflow/create/diagram", summarizeBlueprints_controller],
  [
    Router.get,
    "/workflow/create/diagram/:blueprint_name",
    generateBlueprintDiagram_controller,
  ],
];

const status_route_tuples = [["/status/page/:code", statusPage_controller]];

const routers_tuples = [workflow_route_tuples, status_route_tuples];

let router, verb, route, route_callback;
routers = [];

routers_tuples.forEach((router_tuples) => {
  router = Router();

  router_tuples.forEach((router_tuple) => {
    verb = router_tuple[0];
